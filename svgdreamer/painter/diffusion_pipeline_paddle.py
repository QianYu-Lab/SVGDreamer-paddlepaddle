...
from ppdiffusers.pipelines.stable_diffusion.pipeline_stable_diffusion import (
    rescale_noise_cfg, retrieve_timesteps)
from svgdreamer.diffusers_warp import init_StableDiffusion_pipeline_paddle
...
class DiffusionPipeline_Paddle(paddle.nn.Layer):

    def __init__(self, model_cfg: DictConfig, diffuser_cfg: DictConfig, device: str):
        super().__init__()
        self.device = device

        pipe_kwargs = {
            "torch_dtype": paddle.float32,
        ...
        self.sd_pipeline = init_StableDiffusion_pipeline_paddle(
            model_cfg.model_id,
    ...
    def encode_prompt(self,
        ...
        if do_classifier_free_guidance:
            ...
            concat_prompt_embeds = paddle.concat([negative_prompt_embeds, prompt_embeds])
        ...
        for location in from_where:
            for item in attention_maps[f"{location}_{'cross' if is_cross else 'self'}"]:
                if item.shape[1] == num_pixels:
                    ...
                    reshaped_item = paddle.reshape(item, [len(prompts), -1, res, res, item.shape[-1]])
                    cross_maps = reshaped_item[select]
    ...
    def sampling(self,
        ...
        self.sd_pipeline.set_progress_bar_config(desc='Diffusion Sampling')
        ...
        if not output_type == "latent":
            ...
            image, has_nsfw_concept = self.sd_pipeline.run_safety_checker(image, prompt_embeds.dtype)
        else:
            image = latents
