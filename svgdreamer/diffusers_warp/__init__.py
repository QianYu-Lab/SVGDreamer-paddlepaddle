...
from ppdiffusers import UNet2DConditionModel as UNet2DConditionModel_paddle
...
def init_StableDiffusion_pipeline_paddle(model_id: AnyStr,
                                         custom_pipeline: StableDiffusionPipeline_paddle,
                                         custom_scheduler: SchedulerMixin_paddle = None,
                                         device: str = "cuda",
                                         torch_dtype: paddle.dtype = paddle.float32,
                                         local_files_only: bool = True,
                                         force_download: bool = False,
                                         resume_download: bool = False,
                                         ldm_speed_up: bool = False,
    ...
    if custom_scheduler is not None:
        ...
        pipeline = custom_pipeline.from_pretrained(
            model_id,
            paddle_dtype=torch_dtype,
            local_files_only=local_files_only,
            force_download=force_download,
            resume_download=resume_download,
        ).to(device)

    print(f"load ppdiffusers pipeline: {model_id}")
